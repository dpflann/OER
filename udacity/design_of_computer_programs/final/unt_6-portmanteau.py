...
def natalie(words):
    ...
    # keep track of all portmanteaus (start, mid, end)
    portmanteaus = set([])
    # iterate through each word
    for w_1 in words:
        # pair word with all other words in list, remove w_1 from list
        copy_words = words.copy()
        copy_words.remove(w_1)
        for w_2 in copy_words:
            splits = split_word(w_1)
            for start, middle in splits:
                if w_2[:len(middle)] == middle and w_2[len(middle):]:
                    portmanteaus.add((start, middle, w_2[len(middle):]))
    # score the portmanteaus and return the max
    best_portmanteau = max(portmanteaus, key=score_portmaneau) if portmanteaus else None
    return ''.join(best_portmanteau) if best_portmanteau else None


def score_portmaneau(portmaneau):
    s, m, e = portmaneau
    l = len(''.join(portmaneau))
    return l - abs(len(s) - (l/4.)) - abs(len(m) - (l/2.)) - abs(len(e) - (l/4.))


def split_word(word):
    """Return splits of word where a split is word[0:i] and word[i:] such that word[0:i] + word[i:] == word
    eskimo --> [('e', 'skimo'), ..., ('eskim', 'o')] Leaving off ('', w) and (w, '')
    """
